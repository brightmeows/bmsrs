...
use bmson_def::{DetectedVersion, detect_version};

#[test]
fn detect_v2_from_full_json() {
    let json = r#"{"version":"2.0.0","song_info":{},"chart_info":{},"chart_data":{}}"#;
    assert_eq!(detect_version(json).unwrap(), DetectedVersion::V2);
}
...
fn detect_v1_from_full_json() {
    ...
    assert_eq!(detect_version(json).unwrap(), DetectedVersion::V1);
}
...
fn detect_v0_when_no_version_field() {
    ...
    assert_eq!(detect_version(json).unwrap(), DetectedVersion::V0);
}
